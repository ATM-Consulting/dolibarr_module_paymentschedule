...
if ($search_month > 0) {
	...
	else $sql.= " AND date_format(f.datef, '%m') = '".$search_month."'";
...
if ($search_month_lim > 0) {
	...
	elseif ($search_year_lim > 0 && ! empty($search_day_lim))
	...
	else $sql.= " AND date_format(f.date_lim_reglement, '%m') = '".$db->escape($search_month_lim)."'";
} elseif ($search_year_lim > 0) {
