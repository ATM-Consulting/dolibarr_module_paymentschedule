...
class PaymentSchedule extends SeedObject
{
	...
	/**
	 * Fetch object and children from database
	 *
	 * @param int  $id        Object id
	 * @param bool $loadChild Load child tables
	...
	/**
	 * Get object and children from database based on a custom field
	 *
	 * @param mixed  $key       Value to search
	...
	/**
	 * Fetch object by invoice reference
	 *
	...
	public static function libStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statusType='status6'; $statusLabel=$langs->trans('PaymentScheduleStatusDraft'); $statusLabelShort=$langs->trans('PaymentScheduleStatusDraftShort'); } elseif ($status==self::STATUS_VALIDATED) { $statusType='status4'; $statusLabel=$langs->trans('PaymentScheduleStatusValidated'); $statusLabelShort=$langs->trans('PaymentScheduleStatusValidateShort'); } elseif ($status==self::STATUS_CLOSED) { $statusType='status6'; $statusLabel=$langs->trans('PaymentScheduleStatusClosed'); $statusLabelShort=$langs->trans('PaymentScheduleStatusClosedShort'); }
	...
	/**
	 * Check if an invoice meets conditions for a payment schedule
	 *
	 * @param Facture $facture Invoice object
	 * @return string[]        Array of error messages (empty if OK)
	 */
	public static function checkFactureCondition($facture)
	{
		...
		foreach ($facture->lines as $line) {
			...
			if (isModEnabled('subtotal')) { // Pas de vérification de TVA sur les lignes de sous total
	...
	/**
	 * Create schedule from an invoice
	 *
	 * @param Facture $facture           Invoice object
	 * @param int     $date_start        Start date timestamp
	 * @param string  $periodicity_unit  Unit (day, month, year)
	 * @param int     $periodicity_value Value for unit
	 * @param int     $nb_term           Number of installments
	 * @param bool    $notrigger         Disable triggers
	 * @return int                       ID of schedule or <0
	 */
	 ...
	 *
	 * @param bool   $reset       Delete existing lines
	 * @param string $fill_amount Strategy for rounding (onfirst, onlast)
	 * @return int                Number of lines created or <0
	 */
	public function initDetailEcheancier($reset = false, $fill_amount = 'onlast')
	{
		...
		foreach ($TDatesEcheance as $i => $time) {
			...
			} elseif ($fill_amount === 'onlast' && $i == ($this->nb_term - 1)) {
				$det->amount_ht  = $TLeftAmountToPay['HT'];
				$det->amount_tva = $TLeftAmountToPay['VAT'];
				$det->amount_ttc = $TLeftAmountToPay['TTC'];
			} else {
	...
	/**
	 * Generate clickable link of object
	 *
	 * @param int    $withpicto             Add picto
	 * @param string $option                Option link
	 * @param int    $max                   Max length
	 * @param int    $short                 Just URL
	 * @param string $moretitle             Tooltip text
	 * @param int    $notooltip             Disable tooltip
	 * @param int    $addlinktonotes        Link to notes
...
class PaymentScheduleDet extends SeedObject
{
	...
	/**
	 * Fetch object by direct debit request ID
	 *
	 * @param int $fk_prelevement_facture_demande ID of the withdrawal request
	 * @return int                            >0 if OK, <0 if KO, 0 if not found
	 */
	...
	/**
	 * Set status to in process and link to withdrawal request
	 *
	 * @param User $user                            User object
	 * @param int  $fk_prelevement_facture_demande  ID of linked withdrawal request
	 * @param bool $notrigger                       false=launch triggers after, true=disable triggers
	 * @return int                                  >0 if OK, <0 if KO
	 */
	...
	/**
	 * Set status to requested and link to withdrawal receipt (bon de prelevement)
	 *
	 * @param User $user                User object
	 * @param int  $fk_prelevement_bons ID of linked withdrawal receipt
	 * @param bool $notrigger           false=launch triggers after, true=disable triggers
	 * @return int                      >0 if OK, <0 if KO
	 */
	...
	/**
	 * Get all schedule details from a withdrawal receipt
	 *
	 * @param BonPrelevement $object Withdrawal receipt object
	 * @return array                 Array of PaymentScheduleDet objects
	 */
	...
	/**
	 * Load linked objects with module check override
	 *
	 * @param int    $sourceid        Source ID
	 * @param string $sourcetype      Source type
	 * @param int    $targetid        Target ID
	 * @param string $targettype      Target type
	 * @param string $clause          'OR' or 'AND'
	 * @param int    $alsosametype    Also same type
	...
	/**
	 * Get the payment object from a detail ID using cache
	 *
	 * @param int $fk_paymentdet Row ID in paiement_facture
	 * @return Paiement|false    Paiement object or false if error
	 */
...
/**
 * Class PaymentScheduleUpdateStatus
 *
 * Class to update the status of payment schedules via cron job.
 */
class PaymentScheduleUpdateStatus extends SeedObject
{
	public $output = '';
	/**
	 * Run the scheduled job to update statuses
	 *
	 * This method is called by the Dolibarr cron setup.
	 * It checks for direct debit receipts (bons de prélèvement) in a specific status
	 * and updates them if the date has passed.
	 *
	 * @return int 0 if OK, >0 if KO
	 */
	public function run()
...
/**
 * Class PaymentScheduleBonPrelevement
 * * Overload of BonPrelevement to add specific invoice list retrieval
 */
class PaymentScheduleBonPrelevement extends BonPrelevement
{
	/**
	 * Get list of invoices linked to the withdrawal receipt
	 *
	 * Returns all invoices presented within a withdrawal receipt.
	 *
	 * @param  int   $amounts   1 if you want to get the amount for each invoice, 0 for IDs only
	 * @return array            Array of invoice IDs or array(id, amount, fk_prelevement_lignes)
	 */
